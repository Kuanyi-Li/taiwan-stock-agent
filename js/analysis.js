...
row.innerHTML = items.map(it => ` <div class="ind-card"> <div class="ind-name">${it.name}</div> <div class="ind-value" style="color:${it.color}">${it.value}</div> <span class="ind-signal ${it.signal.cls}">${it.signal.label}</span> </div>`).join(’’);
},
