...
const Calendar = {
  // 2026年FOMC會議時程（公開資訊，Fed官方公布，每年更新一次）
  FOMC_2026: [
  ...
  // 台股法定財報申報截止日（適用所有上市櫃公司，非個股實際公告日，僅供參考）
  TW_FILING_DEADLINES: (year) => [
    { date: `${year}-03-31`, label: `${year-1}年報（Q4）申報截止` },
    { date: `${year}-05-15`, label: `${year}年Q1季報申報截止` },
    { date: `${year}-08-14`, label: `${year}年上半年報申報截止` },
    { date: `${year}-11-14`, label: `${year}年Q3季報申報截止` },
  ],

  getUpcomingFOMC() {
    const today = new Date().toISOString().slice(0, 10);
    return this.FOMC_2026
      .filter(m => m.end >= today)
      .map(m => ({ type: 'fomc', date: m.start, endDate: m.end, label: 'FOMC 利率決策會議', code: null }));
  },

  getUpcomingTWDeadlines() {
    const today = new Date().toISOString().slice(0, 10);
    const year = new Date().getFullYear();
    const all = [...this.TW_FILING_DEADLINES(year), ...this.TW_FILING_DEADLINES(year + 1)];
    return all
      .filter(d => d.date >= today)
      .map(d => ({ type: 'tw-filing', date: d.date, label: d.label, code: null }))
      .slice(0, 3);
  },

  // 美股個股財報日：掃描未來N天的 NASDAQ 財報日曆，比對持股/自選股代碼
  // 結果快取（localStorage，3天TTL），避免每次開網站都重新掃描
  _cacheKey() { return 'ussa-earnings-cache'; },
  async getUpcomingUSEarnings(symbols) {
    if (!symbols?.length) return [];
    const cacheRaw = localStorage.getItem(this._cacheKey());
    if (cacheRaw) {
      try {
        const cache = JSON.parse(cacheRaw);
        if (Date.now() - cache.ts < 3 * 86400000) return cache.events.filter(e => symbols.includes(e.code));
    ...
    for (let i = 0; i < 45; i++) {
      ...
      try {
        ...
        (json?.data?.rows || []).forEach(r => {
          if (symSet.has((r.symbol || '').toUpperCase())) {
            events.push({ type: 'earnings', date: dateStr, label: `${r.name || r.symbol} 財報公布`, code: r.symbol });
          }
        });
      } catch(e) { /* 靜默跳過該天 */ }
  ...
  async getAll() {
    const usSymbols = [
      ...APP._usPortfolio.map(s => s.code),
      ...APP._usWatchlist.map(s => s.code),
    ];
    const [fomc, twDeadlines, usEarnings] = await Promise.all([
      Promise.resolve(this.getUpcomingFOMC()),
      Promise.resolve(this.getUpcomingTWDeadlines()),
      this.getUpcomingUSEarnings(usSymbols),
    ]);
    return [...fomc, ...twDeadlines, ...usEarnings].sort((a, b) => a.date.localeCompare(b.date));
  },
  ...
  async openModal() {
    ...
    if (!events.length) { list.innerHTML = '<div class="empty-state">近期無重大財經事件</div>'; return; }
    const typeIcon = { fomc: '🏛️', 'tw-filing': '📋', earnings: '📊' };
    const typeColor = { fomc: '#eab308', 'tw-filing': '#37adf0', earnings: '#E24B4A' };
    list.innerHTML = events.map(e => {
      const daysLeft = Math.ceil((new Date(e.date) - new Date()) / 86400000);
      return `
        <div class="cal-item">
          <div class="cal-icon" style="color:${typeColor[e.type]}">${typeIcon[e.type]}</div>
          <div class="cal-body">
            <div class="cal-label">${e.label}${e.code ? ` <span class="cal-code">${e.code}</span>` : ''}</div>
            <div class="cal-date">${e.date}${e.endDate ? `～${e.endDate}` : ''} · ${daysLeft <= 0 ? '今天' : `${daysLeft}天後`}</div>
          </div>
        </div>`;
    }).join('');
  },
