...
return `<div class="trade-item"> <div class="ti-left"> <span class="ti-action ${isBuy?'buy':'sell'}">${isBuy?'買進':'賣出'}</span> <span class="ti-code">${t.code}</span> <span class="ti-name">${t.name}</span> </div> <div class="ti-mid"> <span>${sharesDisplay} @ $${t.price}</span> <span class="ti-date">${t.date || '—'}</span> </div> <div class="ti-right"> <span class="${isBuy?'dn-color':'up-color'}">${isBuy?'-':'+'}${totalDisplay}</span> ${fee ? `<span class="ti-fee">稅費 $${fee}</span>` : ''} </div> </div>`;
}).join(’’);
},
};
...
tr.innerHTML = ` <td class="td-batch">${batches>1?`第${i+1}批`:'進場'}<br><small style="color:var(--text-3);font-size:10px">預算${(budget*ratio/10000).toFixed(1)}萬</small></td> <td class="td-price">$${batchPrice}</td> <td class="td-shares">${sharesDisp}</td> <td class="td-amount">${costDisp}</td> <td class="td-fee" style="color:var(--text-3);font-size:11px">+${fee}</td> <td class="td-pct">${(ratio*100).toFixed(0)}%</td>`;
...
orderHint = `<div class="order-hint" style="margin-top:6px;font-size:11px;padding:6px 8px;background:var(--bg-3);border-radius:6px;border-left:3px solid ${hintColor}"> 📋 掛單參考：<span style="color:${hintColor};font-weight:600">${action}</span> ${totalShares}股 @ <strong>$${this.suggestEntry}</strong> ｜停損 $${this.suggestSL} ｜停利 $${this.suggestTP} </div>`;
}
}
...
return `<div class="sig-overview-item ${sig.cls}" onclick="APP.selectStock('${s.code}', ${this.portfolio.indexOf(s)}, 'portfolio')"> <div class="soi-left"> <div class="soi-code">${s.code}</div> <div class="soi-name">${s.name}</div> </div> <div class="soi-mid"> <div class="soi-price">${s.price ? s.price.toFixed(2) : '—'}</div> <div class="soi-gain ${gainPct>=0?'up-color':'dn-color'}">${gainDisp}</div> </div> <div class="soi-right"> <div class="soi-signal-label ${sig.cls}">${sig.label}</div> <div class="soi-signal-short">${sig.short}</div> ${daysHeld !== null ?`<div class="soi-days">持有${daysHeld}天</div>` : ''} </div> </div>`;
}).join(’’);
},
...
div.innerHTML = `<div class="wi-left" onclick="APP.selectStock('${s.code}',${i},'watch')"> <div class="wi-code">${s.code}</div> <div class="wi-name">${s.name}</div> ${sig ?`<div class="wi-signal ${sig.cls}">${sig.short} ${sig.label}</div>` : ''} </div> <div class="wi-right" onclick="APP.selectStock('${s.code}',${i},'watch')"> <div class="wi-price ${isUp?'up-color':'dn-color'}">${price>0?price.toFixed(2):'—'}</div> <div class="wi-change ${isUp?'up-color':'dn-color'}">${price>0&&Math.abs(chg)>0.001?(isUp?'+':'')+chgPct.toFixed(2)+'%':''}</div> </div> <button class="watch-del" onclick="APP.removeWatch(${i})">✕</button>`;
...
wrap.innerHTML = signals.map(s => ` <div class="sell-signal-item ${s.urgency}"> <div class="ss-icon ${s.urgency}">${icons[s.urgency]??'•'}</div> <div class="ss-body"><div class="ss-label ${s.urgency}">${s.label}</div><div class="ss-desc">${s.desc}</div></div> </div>`).join(’’);
}
}
